...
```bash
node scripts/zhima-credit-payafteruse.js sign \
  --out_agreement_no "AGR001" \
  --zm_service_id "您的服务ID"
```

### 3. 创建订单

```bash
node scripts/zhima-credit-payafteruse.js order \
  --credit_agreement_id "开通协议号" \
  --out_order_no "ORD001" \
  --subject "订单标题"
```

> 📖 完整流程请参阅 [业务流程详解](references/workflow.md)
...
> 📖 详细引导规范请参阅 [交互引导规范](references/interaction-guide.md)
...
> 💡 详细参数说明请参阅 [API 参考文档](references/api-reference.md)
...
> 📖 详细的流程图及命令示例请参阅 [业务流程详解](references/workflow.md)

## 📎 文档导航

### 我要开始使用

- [业务流程详解](references/workflow.md) — 了解免确认/需确认两种模式
- [API 参考文档](references/api-reference.md) — 查看命令格式和参数
- [配置文件示例](references/config.json.example) — 密钥配置模板

### 我要查看示例

- [示例报文目录](references/examples/) — 9个接口的完整报文示例
- [示例报文校验](references/example-guide.md) — 校验我的请求报文

### 我遇到了问题

- [常见问题](references/faq.md) — 错误码和解决方案
- [字段词典](references/glossary.md) — 字段含义和流转关系

### 开发者资源

- [交互引导规范](references/interaction-guide.md) — 交互式引导话术和流程规范
- [评测方案](tests/test-cases.md) — Skill评测标准和用例
- [评测报告](tests/test-report.md) — 最新评测结果
...
> 📖 更多问题请参阅 [常见问题](references/faq.md)
